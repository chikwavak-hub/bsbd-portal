          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 14, flexWrap: 'wrap' }}>
            {Object.entries(TAB_LABELS).map(([k, l]) => (
              <button key={k} onClick={() => setTab(k)}
                style={{ padding: '6px 14px', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 600, background: tab === k ? '#0d9488' : 'white', color: tab === k ? 'white' : '#64748b', border: tab === k ? 'none' : '1px solid #e2e8f0' }}>
                {l}
              </button>
            ))}
          </div>
